...
static int ulite_probe(struct platform_device *pdev)
{
	...
	int irq;
	int id = pdev->id;
	printk(KERN_INFO"uartlite: calling ulite_probe()\n");
#ifdef CONFIG_OF
	const __be32 *prop;
...
static int __init ulite_init(void)
{
	...
	printk(KERN_INFO"uartlite: calling uart_register_driver()\n");
	ret = uart_register_driver(&ulite_uart_driver);
	if (ret)
		goto err_uart;

	printk(KERN_INFO"uartlite: calling platform_driver_register()\n");
